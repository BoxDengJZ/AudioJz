...
class AssetPlaybackManager: NSObject {
    ...
    func skipForward(_ interval: TimeInterval) {
        ...
        player.seek(to: newTime, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero, completionHandler: { (_) in
    ...
    func skipBackward(_ interval: TimeInterval) {
        ...
        player.seek(to: newTime, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero) { (_) in
            self.updatePlaybackRateMetadata()
        }
