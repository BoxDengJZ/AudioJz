...
open class NodeRecorder: NSObject {
    ...
    open var audioFile: AVAudioFile? {
        do {
            ...
            print("Error, Cannot create internal audio file for reading: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    public init(node: Node,
        ...
        guard let file = file else {
            print("Error, no file to write to")
            return
        }
        ...
        do {
            ...
            print("Error: cannot write to", file.url)
            throw error
        }
    ...
    public static func removeTempFiles() {
        for url in NodeRecorder.tmpFiles {
            ...
            print("𝗫 Deleted tmp file at", url)
        }
    ...
    public func stop() {
        if isRecording == false {
            print("Warning: Cannot stop recording, already stopped")
            return
        }
